...
export function RunnerPanel({ runner, onRestart, onClose, input, onInputChange, onSubmit }) {
  ...
  return (
    <section className="fixed bottom-5 left-1/2 z-30 flex h-[460px] w-[420px] -translate-x-1/2 flex-col rounded-lg border border-slate-200 bg-white shadow-[0_28px_90px_rgba(15,23,42,0.32)]">
      <div className="flex items-center justify-between gap-3 border-b border-slate-200 px-4 py-3">
        ...
        <div className="flex gap-2">
          <button type="button" onClick={onRestart} className="grid h-8 w-8 place-items-center rounded-md border border-slate-200 text-slate-600 hover:bg-slate-50" title="Restart runner">
            <RotateCcw size={15} />
          </button>
          <button type="button" onClick={onClose} className="grid h-8 w-8 place-items-center rounded-md border border-slate-200 text-slate-600 hover:bg-slate-50" title="Close runner">
            <X size={16} />
          </button>
        </div>
      </div>
      ...
      <form onSubmit={onSubmit} className="border-t border-slate-200 p-3">
        <div className="flex gap-2">
          <input
            ...
            placeholder={runner.status === 'waiting' ? 'Type your answer...' : 'Runner is not waiting for input'}
          ...
          />
          <button type="submit" disabled={runner.status !== 'waiting'} className="h-10 rounded-md bg-slate-950 px-4 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:bg-slate-300">
            Send
          </button>
        </div>
      </form>
